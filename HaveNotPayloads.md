# APISI OLMAYAN PAGE DATALARI

## Dashboard Page

Dashboard KPI Metrikleri icin henuz bir API yok. Dummy response icin kullanilan yapi asagidaki gibidir.

```json
"data": [{
    "connector": {
        "title": "Toplam Konnektor",
        "value": "450/483",
        "icon_name": "GiElectricalSocket",
        "description": "This is description of this card",
        "position": "1 / 9 / 7 / 13",
        "mobile_layout": "16 / 1 / 16 / 12",
        "tablet_layout": "5 / 1 / 5 / 5",
        "type": ""
    },
    "ac_status": {
        "title": "AC Sayısı",
        "value": "350/371",
        "icon_name": "PiWaveSineBold",
        "description": "This is description of this card",
        "position": "7 / 9 / 14 / 13",
        "mobile_layout": "17 / 1 / 17 / 12",
        "tablet_layout": "6 / 1 / 6 / 3",
        "type": ""
    },
    "dc_status": {
        "title": "DC Sayısı",
        "value": "100/112",
        "icon_name": "IoIosFlash",
        "description": "This is description of this card",
        "position": "14 / 9 / 21 / 13",
        "mobile_layout": "18 / 1 / 18 / 12",
        "tablet_layout": "6 / 3 / 6 / 5",
        "type": ""
    }
}],
"message": "",
"status": 200
```

## Seconda Dashboard Page

Dashboard KPI Metrikleri icin henuz bir API yok. Dummy response icin kullanilan yapi asagidaki gibidir.

```json
"data": [{
    "total_charge_process_amount": {
        "title": "Toplam Şarj İşlem Tutari",
        "value": "10000 / 11012",
        "icon_name": "FaChargingStation",
        "description": "Tüm zamanlarin toplam sarj islem tutari",
        "position": "1 / 1 / 3 / 5",
        "mobile_layout": "1 / 1 / 1 / 12",
        "tablet_layout": "1 / 1 / 1 / 5",
        "type": "",

    },
    "last_eight_days_ac_dc_sales": {
        "title": "Son 8 Gun AC DC Satışları",
        "value": [
            { "ac": [{ "1": 100 }, { "2": 300 }, { "3": 200 }, { "4": 250 }, { "5": 150 }, { "6": 220 }, { "7": 400 }, { "8": 250 }] },
            { "dc": [{ "1": 300 }, { "2": 250 }, { "3": 350 }, { "4": 400 }, { "5": 200 }, { "6": 300 }, { "7": 450 }, { "8": 100 }] }
        ],
        "icon_name": "",
        "description": "Son 8 gun icinde AC ve DC satislarinin karsilastirmasi",
        "position": "1 / 5 / 3 / 9",
        "mobile_layout": "2 / 1 / 2 / 12",
        "tablet_layout": "1 / 5 / 3 / 9",
        "type": "line"
    },
    "total_customer_count": {
        "title": "Müşteri Sayısı",
        "value": "700/765",
        "icon_name": "HiUserGroup",
        "description": "Tüm zamanlarin toplam müşteri sayısı",
        "position": "1 / 9 / 3 / 13",
        "mobile_layout": "3 / 1 / 3 / 12",
        "tablet_layout": "1 / 9 / 2 / 12",
        "type": ""
    },
    "total_used_kwh": {
        "title": "Toplam Kwh Tüketimi",
        "value": "10000",
        "icon_name": "FaBolt",
        "description": "",
        "position": "3 / 1 / 3 / 3",
        "mobile_layout": "6 / 1 / 6 / 12",
        "tablet_layout": "3 / 1 / 3 / 3",
        "type": ""
    },
    "total_charge_process_count": {
        "title": "Toplam Şarj İşlem Sayısı",
        "value": "1000/1100",
        "icon_name": "",
        "description": "Gun icinde yapilan sarj sayısı",
        "position": "3 / 3 / 3 / 5",
        "mobile_layout": "7 / 1 / 7 / 12",
        "tablet_layout": "3 / 4 / 3 / 6",
        "type": ""
    },
    "payment_method_types": {
        "title": "Ödeme Yontemleri",
        "value": [
            {
                "Kredi Karti (MP)": 100
            }, {
                "Sharz Card": 200
            }, {
                "Fleet / Wallet": 300
            }
        ],
        "icon_name": "icon_name",
        "description": "Yapilan odeme yontemlerinin karsilastirmasi",
        "position": "3 / 5 / 5 / 7",
        "mobile_layout": "8 / 1 / 8 / 12",
        "tablet_layout": "3 / 7 / 5 / 9",
        "type": "doughnut"
    },
    "ac_dc_sales": {
        "title": "AC/DC Satış Tutarlari",
        "value": [{
            "AC": "100"
        }, {
            "DC": "200"
        }],
        "icon_name": "icon_name",
        "description": "Gun icindeki AC ve DC satislarinin karsilastirmasi",
        "position": "3 / 7 / 5 / 9",
        "mobile_layout": "9 / 1 / 9 / 12",
        "tablet_layout": "3 / 10 / 5 / 12",
        "type": "semi_doughnut"
    },
    "last_three_months_active_customer_count": {
        "title": "Aktif Müşteri Sayısı",
        "value": "600/736",
        "icon_name": "icon_name",
        "description": "Son 3 ay icinde sarj islemi yapmis kullanıcı sayısı",
        "position": "3 / 9 / 3 / 11",
        "mobile_layout": "10 / 1 / 10 / 12",
        "tablet_layout": "3 / 9 / 3 / 11",
        "type": ""
    },
    "inactive_customer_count": {
        "title": "Pasif Müşteri Sayısı",
        "value": "136/736",
        "icon_name": "icon_name",
        "description": "Son 3 ay icinde sarj islemi yapmamis kullanıcı sayısı",
        "position": "3 / 11 / 3 / 13",
        "mobile_layout": "11 / 1 / 11 / 12",
        "tablet_layout": "3 / 11 / 3 / 12",
        "type": ""
    },
    "total_cost": {
        "title": "Toplam Maliyet",
        "value": "10000/15563",
        "icon_name": "icon_name",
        "description": "Tüm zamanlarin toplam maliyeti",
        "position": "4 / 1 / 4 / 3",
        "mobile_layout": "12 / 1 / 12 / 12",
        "tablet_layout": "4 / 1 / 4 / 3",
        "type": ""
    },
    "total_earn": {
        "title": "Toplam Kazanc",
        "value": "15000/20000",
        "icon_name": "icon_name",
        "description": "Tüm zamanlarin toplam kazanci",
        "position": "4 / 3 / 4 / 5",
        "mobile_layout": "13 / 1 / 13 / 12",
        "tablet_layout": "4 / 3 / 4 / 5",
        "type": ""
    },
    "last_month_active_customer_count": {
        "title": "Son Ay Aktif Müşteri Sayısı",
        "value": "100/123",
        "icon_name": "icon_name",
        "description": "Son 1 ayda sarj islemi yapmis kullanıcı sayısı",
        "position": "4 / 9 / 4 / 11",
        "mobile_layout": "14 / 1 / 14 / 12",
        "tablet_layout": "4 / 9 / 4 / 11",
        "type": ""
    },
    "last_week_active_customer_count": {
        "title": "Son Hafta Aktif Müşteri Sayısı",
        "value": "50/68",
        "icon_name": "icon_name",
        "description": "Son 1 hafta icinde sarj islemi yapmis kullanıcı sayısı",
        "position": "4 / 11 / 4 / 13",
        "mobile_layout": "15 / 1 / 15 / 12",
        "tablet_layout": "4 / 11 / 4 / 12",
        "type": ""
    },
    "daily_income": {
        "title": "Günlük İşlem Tutarı",
        "value": [
            { "today": [{ "00": 100 }, { "01": 300 }, { "02": 200 }, { "03": 250 }, { "04": 150 }, { "05": 220 }, { "06": 400 }, { "07": 250 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] },
            { "last_week_today": [{ "00": 100 }, { "01": 300 }, { "02": 250 }, { "03": 350 }, { "04": 400 }, { "05": 200 }, { "06": 300 }, { "07": 450 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] }
        ],
        "icon_name": "icon_name",
        "description": "Gunluk islem tutarinin karsilastirmasi",
        "position": "5 / 1 / 7 / 3",
        "mobile_layout": "20 / 1 / 20 / 12",
        "tablet_layout": "5 / 1 / 7 / 3",
        "type": "line"
    },
    "daily_kwh_usage": {
        "title": "Günlük Kwh Tüketimi",
        "value": [
            { "today": [{ "00": 100 }, { "01": 300 }, { "02": 200 }, { "03": 250 }, { "04": 150 }, { "05": 220 }, { "06": 400 }, { "07": 250 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] },
            { "last_week_today": [{ "00": 100 }, { "01": 300 }, { "02": 250 }, { "03": 350 }, { "04": 400 }, { "05": 200 }, { "06": 300 }, { "07": 450 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] }
        ],
        "icon_name": "icon_name",
        "description": "Gunluk kwh Tüketiminin karsilastirmasi",
        "position": "5 / 3 / 7 / 5",
        "mobile_layout": "21 / 1 / 21 / 12",
        "tablet_layout": "5 / 3 / 7 / 5",
        "type": "line"
    },
    "monthly_income": {
        "title": "Aylık Kazanç",
        "value": [
            { "month": [{ "00": 100 }, { "01": 300 }, { "02": 200 }, { "03": 250 }, { "04": 150 }, { "05": 220 }, { "06": 400 }, { "07": 250 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] },
            { "last_month": [{ "00": 100 }, { "01": 300 }, { "02": 250 }, { "03": 350 }, { "04": 400 }, { "05": 200 }, { "06": 300 }, { "07": 450 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] }
        ],
        "icon_name": "icon_name",
        "description": "Aylik kazancin karsilastirmasi",
        "position": "5 / 5 / 7 / 7",
        "mobile_layout": "22 / 1 / 22 / 12",
        "tablet_layout": "5 / 5 / 7 / 7",
        "type": "line"
    },
    "monthly_kwh_usage": {
        "title": "Aylık Kwh Tüketimi",
        "value": [
            { "month": [{ "00": 100 }, { "01": 300 }, { "02": 200 }, { "03": 250 }, { "04": 150 }, { "05": 220 }, { "06": 400 }, { "07": 250 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] },
            { "last_month": [{ "00": 100 }, { "01": 300 }, { "02": 250 }, { "03": 350 }, { "04": 400 }, { "05": 200 }, { "06": 300 }, { "07": 450 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] }
        ],
        "icon_name": "icon_name",
        "description": "Aylik kwh Tüketiminin karsilastirmasi",
        "position": "5 / 7 / 7 / 9",
        "mobile_layout": "23 / 1 / 23 / 12",
        "tablet_layout": "5 / 7 / 7 / 9",
        "type": "line"
    },
    "yearly_income": {
        "title": "Yıllık Kazanç",
        "value": [
            { "year": [{ "00": 100 }, { "01": 300 }, { "02": 200 }, { "03": 250 }, { "04": 150 }, { "05": 220 }, { "06": 400 }, { "07": 250 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] },
            { "last_year": [{ "00": 100 }, { "01": 300 }, { "02": 250 }, { "03": 350 }, { "04": 400 }, { "05": 200 }, { "06": 300 }, { "07": 450 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] }
        ],
        "icon_name": "icon_name",
        "description": "Yillik kazancin karsilastirmasi",
        "position": "5 / 9 / 7 / 11",
        "mobile_layout": "24 / 1 / 24 / 12",
        "tablet_layout": "5 / 9 / 7 / 11",
        "type": "line"
    },
    "yearly_kwh_usage": {
        "title": "Yıllık Kwh Tüketimi",
        "value": [
            { "year": [{ "00": 100 }, { "01": 300 }, { "02": 200 }, { "03": 250 }, { "04": 150 }, { "05": 220 }, { "06": 400 }, { "07": 250 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] },
            { "last_year": [{ "00": 100 }, { "01": 300 }, { "02": 250 }, { "03": 350 }, { "04": 400 }, { "05": 200 }, { "06": 300 }, { "07": 450 }, { "08": 100 }, { "09": 100 }, { "10": 300 }, { "11": 200 }, { "12": 250 }, { "13": 150 }, { "14": 220 }, { "15": 400 }, { "16": 250 }, { "17": 100 }, { "18": 100 }, { "19": 300 }, { "20": 200 }, { "21": 250 }, { "22": 150 }, { "23": 220 }] }
        ],
        "icon_name": "icon_name",
        "description": "Yillik kwh Tüketiminin karsilastirmasi",
        "position": "5 / 11 / 7 / 13",
        "mobile_layout": "25 / 1 / 25 / 12",
        "tablet_layout": "5 / 11 / 7 / 12",
        "type": "line"
    },
    "monthly_total": {
        "title": "Aylık Toplam",
        "value": [
            { "08-2023": { "amount": 5190, "charge_count": 4000, "kwh": 90000, "service_fee": 143000 } },
            { "09-2023": { "amount": 3190, "charge_count": 3000, "kwh": 80000, "service_fee": 150000 } },
            { "10-2023": { "amount": 3019, "charge_count": 2590, "kwh": 75000, "service_fee": 125000 } },
            { "11-2023": { "amount": 3590, "charge_count": 3001, "kwh": 83200, "service_fee": 149000 } },
            { "12-2023": { "amount": 3500, "charge_count": 3250, "kwh": 91000, "service_fee": 139000 } },
            { "01-2024": { "amount": 3190, "charge_count": 1459, "kwh": 86000, "service_fee": 125999 } },
            { "02-2024": { "amount": 1900, "charge_count": 3698, "kwh": 79000, "service_fee": 139500 } },
            { "03-2024": { "amount": 1987, "charge_count": 3200, "kwh": 78500, "service_fee": 189623 } },
            { "04-2024": { "amount": 3520, "charge_count": 3190, "kwh": 76000, "service_fee": 198533 } },
            { "05-2024": { "amount": 3654, "charge_count": 4300, "kwh": 71000, "service_fee": 179563 } },
            { "06-2024": { "amount": 2958, "charge_count": 4125, "kwh": 69500, "service_fee": 179963 } },
            { "07-2024": { "amount": 3000, "charge_count": 3009, "kwh": 78900, "service_fee": 169000 } },
            { "08-2024": { "amount": 3100, "charge_count": 3756, "kwh": 85200, "service_fee": 187653 } }
        ],
        "icon_name": "icon_name",
        "description": "Aylik toplam satislarin karsilastirmasi",
        "position": "7 / 1 / 9 / 7",
        "mobile_layout": "26 / 1 / 28 / 12",
        "tablet_layout": "7 / 1 / 9 / 7",
        "type": "line&bar"
    },
    "monthly_total2": {
        "title": "Yillik Toplam",
        "value": [
            { "08-2023": { "amount": 5190, "charge_count": 4000, "kwh": 90000, "service_fee": 143000 } },
            { "09-2023": { "amount": 3190, "charge_count": 3000, "kwh": 80000, "service_fee": 150000 } },
            { "10-2023": { "amount": 3019, "charge_count": 2590, "kwh": 75000, "service_fee": 125000 } },
            { "11-2023": { "amount": 3590, "charge_count": 3001, "kwh": 83200, "service_fee": 149000 } },
            { "12-2023": { "amount": 3500, "charge_count": 3250, "kwh": 91000, "service_fee": 139000 } },
            { "01-2024": { "amount": 3190, "charge_count": 1459, "kwh": 86000, "service_fee": 125999 } },
            { "02-2024": { "amount": 1900, "charge_count": 3698, "kwh": 79000, "service_fee": 139500 } },
            { "03-2024": { "amount": 1987, "charge_count": 3200, "kwh": 78500, "service_fee": 189623 } },
            { "04-2024": { "amount": 3520, "charge_count": 3190, "kwh": 76000, "service_fee": 198533 } },
            { "05-2024": { "amount": 3654, "charge_count": 4300, "kwh": 71000, "service_fee": 179563 } },
            { "06-2024": { "amount": 2958, "charge_count": 4125, "kwh": 69500, "service_fee": 179963 } },
            { "07-2024": { "amount": 3000, "charge_count": 3009, "kwh": 78900, "service_fee": 169000 } },
            { "08-2024": { "amount": 3100, "charge_count": 3756, "kwh": 85200, "service_fee": 187653 } }
        ],
        "icon_name": "icon_name",
        "description": "Yillik toplam satislarin karsilastirmasi",
        "position": "7 / 7 / 9 / 13",
        "mobile_layout": "28 / 1 / 30 / 12",
        "tablet_layout": "7 / 7 / 9 / 12",
        "type": "line&bar"
    },
    "most_used_locations": {
        "title": "En Çok Kullanılan Lokasyonlar",
        "value": [
            { "Location 1": 25 },
            { "Location 2": 20 },
            { "Location 3": 15 },
            { "Location 4": 10 },
            { "Location 5": 5 }
        ],
        "icon_name": "icon_name",
        "description": "En cok kullanilan lokasyonlarin listesi",
        "position": "9 / 1 / 11 / 5",
        "mobile_layout": "30 / 1 / 31 / 12",
        "tablet_layout": "9 / 1 / 11 / 5",
        "type": "list"
    },
    "most_charging_users": {
        "title": "En Çok Şarj Yapan Kullanıcılar",
        "value": [
            { "User 1": 25 },
            { "User 2": 20 },
            { "User 3": 15 },
            { "User 4": 10 },
            { "User 5": 5 }
        ],
        "icon_name": "icon_name",
        "description": "En cok sarj yapan kullanicilarin listesi",
        "position": "9 / 5 / 11 / 9",
        "mobile_layout": "31 / 1 / 32 / 12",
        "tablet_layout": "9 / 5 / 11 / 9",
        "type": "list"
    },
    "most_payment_types": {
        "title": "Ödeme Yöntemleri",
        "value": [
            { "Kredi Kartı": 25 },
            { "Sharz Card": 20 },
            { "Fleet / Wallet": 15 }
        ],
        "icon_name": "icon_name",
        "description": "En cok kullanilan odeme yontemlerinin listesi",
        "position": "9 / 9 / 11 / 13",
        "mobile_layout": "32 / 1 / 33 / 12",
        "tablet_layout": "9 / 10 / 11 / 12",
        "type": "list"
    }
}],
"message": "",
"status": 200
```

## Service Point Details Page

Charge Unit Information icin var olan

```
getChargePointFeatureStatus,
getChargeUnitFeatureValuesRequest,
getConnectorModels
getChargePointFeatures
```

requestlerine sarj unitelerinin resimleri, markalari ve modelleri eklendigi icin update edilecek.

## Campaigns Page

Bu sayfa henuz hicbir dataya sahip degil. Burasi icin sifirdan API ler olusuturulabilir. Mobil ile entegre ilerlenebilir.

## Notifications Page

Mobil icin tanimlanacak notification center sayfasi mevcut su an herhangi bir data yok. Burada olmasi gereken ve beklenene gore API response veya API payloadi tanimlanabilir. Sadece sayfa hazir.

## Devices Page

Şarj cihazi marka/model tanimlama, sarj cihazi resim ekleme sayfasi.

```json
"data": [{
    "devices": [
        {
            "id": "xyzxyz",
            "name": "CIRCONTROL",
            "model": "ABC123",
            "kwValue": 180,
            "imagePath": "/klasor/klasor"
        }
    ]
}],
"message": "",
"status": 200
```

## FAQ Page

Henuz bir hicbir data yok.
