...
const TableHeader: React.FC<ITableHeaderProps> = ({ children, filteredDropdownItems, hasFilterData = true }: ITableHeaderProps) => {
    ...
    return (
        <div className={`${tableHeaderPrefix}-actions-container flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 w-full md:flex-row border-t border-gray-300`}>
            {
                hasFilterData && filteredDropdownItems.length > 0 && (
            ...
            <div className={`${tableHeaderPrefix}-button-container flex relative w-full mx-2 md:w-1/2 lg:w-1/4`}>
                {children}
            </div>
        </div>
    );
