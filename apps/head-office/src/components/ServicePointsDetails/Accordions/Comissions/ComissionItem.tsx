...
const ComissionItem: React.FC<IComissionItemProps> = ({ stationId, comissionDetail }: IComissionItemProps) => {
  ...
  const handleDelete = (): void => {
    openModal(
      ...
      <ConfirmationModal
        ...
        onConfirm={() =>
          deleteComission({
            body: { rid: comissionDetail.RID, stationId, isDelete: 1 },
          })
        }
      />
    );
  ...
  return (
    ...
    >
      <InfoColumn className={`${sectionPrefix}-reseller-container`}>
        {comissionDetail?.OwnerTypeName}
      </InfoColumn>
      ...
      <InfoColumn className={`${sectionPrefix}-percent-container`}>
        {comissionDetail?.Rate}
      </InfoColumn>
