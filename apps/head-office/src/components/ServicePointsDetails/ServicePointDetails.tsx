...
import type { ISlugProps, ITabsItemProps, ITabTitleProps } from './types';
import './ServicePointDetails.css';

const ServicePointsDetails: React.FC<ISlugProps> = ({ slug }: ISlugProps) => {
  const isMobile: boolean = useMemo(() => detectDevice().isMobile, []);
  const sectionPrefix: string = `${BRAND_PREFIX}-service-point-details-page-content`;
